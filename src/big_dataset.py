...
from src.config.Constants import DEFAULT_SAVE_TO
    ...
    with open("url_list.json", "r") as url_file:
        for line in tqdm(url_file.readlines()):
            ...
            try:
                ...
                series_row["publish"] = dict_row[
                    "publish"
                ]  # TODO: change to date
                series_row["genres"] = (
                    " ".join(dict_row["genres"])
                    .replace("\n", "")
                    .replace(" ", "")
                    .replace("Category:", "")
                    .split("»")
                ...
                for filename in filenames:
                    ...
                    with open(
                        os.path.join(data_path, filename), encoding="utf8"
                    ) as file:
...
def preprocess() -> pd.DataFrame:
    ...
    data.drop(
        ["publish", "genres", "title", "Unnamed: 0"], axis=1, inplace=True
    )
    values = [i for i in range(0, data["author"].value_counts().sum())]
    key = list(
        dict.fromkeys(list(data["author"]))
    )  # remove duplicate author's name
...
#         [arr, np.zeros(pad_size - arr.size)], dtype=np.float32
#     )
#
#
...
#     return np.concatenate(
#         [arr, np.zeros((max_length - arr.shape[0], arr.shape[1]))],
#         axis=0,
#         dtype=np.float32,
#     )
