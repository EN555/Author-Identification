...
def complex_tranform_word(
    word: str, embedding_table=None
) -> Optional[np.ndarray]:
    embedding_table = _get_embedding_table(embedding_table)
...
def article_level_preprocess(df: pd.DataFrame, embedding_table=None):
    def helper(X):
        X_pre = np.zeros(
            (X.shape[0], MAX_NUMBER_OF_SENTENCE, EMBEDDING_SIZE),
            dtype=np.float32,
        )
            ...
            for i, sentence in enumerate(sentences):
                ...
                for j, word in enumerate(words):
                    ...
                    if (
                        ...
                        and i < MAX_NUMBER_OF_SENTENCE
                        and j < MAX_SENTENCE_LENGTH
                    ):
