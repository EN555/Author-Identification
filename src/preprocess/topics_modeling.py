...
def default_preprocess(df,) -> (corpora.Dictionary, List[Tuple[int, float]]):
    ...
    result = [id2word.doc2bow(text) for text in data_words]
    return id2word, [item for sublist in result for item in sublist]
...
def topic_modeling(
    x_train: pd.DataFrame, x_test: pd.DataFrame, num_topics: int = 5
) -> (pd.Series, pd.Series):
    ...
    pyLDAvis.save_html(
        LDAvis_prepared, f"./ldavis_prepared_{str(num_topics)}.html"
    )
    print(LDAvis_prepared)
    return (
        post_process(lda_model[corpus]),
        post_process(lda_model[default_preprocess(x_test)[1]]),
    )
