...
def simple_style_features_extraction(
    ...
    def simple_style_features_extraction_helper(
        x: pd.DataFrame,
    ) -> pd.DataFrame:
        x = pd.concat(
            [
    ...
    return (
        simple_style_features_extraction_helper(x_train),
        simple_style_features_extraction_helper(x_test),
    )
...
def avg_word_len(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame(avg_word_len_df)
...
def avg_sentence_len(df: pd.DataFrame) -> pd.DataFrame:
    sentence_count = (
        ...
        .astype(str)
        .swifter.apply(
            lambda text: pd.Series(nltk.sent_tokenize(text))
            .map(lambda sent: len(nltk.word_tokenize(sent)))
            .mean()
        )
        .rename("avg_sentence_len")
    )
...
def punctuation_marks_count(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for mark in list(string.punctuation):
        to_return[mark] = (
            df[TEXT_COLUMN_NAME]
            .astype(str)
            .swifter.apply(lambda s: s.count(mark) / len(s))
        )
    return to_return
...
def pos_count(df: pd.DataFrame) -> pd.DataFrame:
    ...
    features = (
        ...
        .swifter.apply(
            lambda s: pd.Series(
                [x[1] for x in nltk.pos_tag(nltk.word_tokenize(s))], dtype=str
            )
            .apply(group_pos)
...
def average_comma_uses(data) -> pd.DataFrame:
    data["sentence_len"] = data["Text"].swifter.apply(
        lambda text: pd.Series(nltk.sent_tokenize(text))
        .map(
            lambda sent: pd.Series(sent).value_counts()[","]
