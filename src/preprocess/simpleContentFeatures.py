...
def name_entity_recognition(df: pd.DataFrame) -> pd.DataFrame:
    ...
    #     "CARDINAL",
    # ]
    name_entity = df["book_text"].swifter.apply(
        lambda s: pd.Series([x.label_ for x in nlp(s).ents]).value_counts()
    )
...
def noun_phrases_extraction(df: pd.DataFrame) -> pd.DataFrame:
    ...
    phrases = df["book_text"].swifter.apply(
        lambda s: pd.Series(nlp(s).noun_chunks)
        .map(lambda c: c.text.lower())
        .value_counts()
    )
...
def bag_of_words(df: pd.DataFrame, cv=None) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        data=res.toarray(), columns=cv.get_feature_names_out()
    ).fillna(value=0)


def simple_content_feature_extraction(
