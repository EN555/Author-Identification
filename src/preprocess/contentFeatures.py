...
def stem_words(text: str) -> List[str]:
    ...
    return [
        lemmatizer.lemmatize(word, PENN2WN.get(pos, wn.NOUN))
...
def word_2_vec(
    ...
    def get_word2vec(text: str):
        ...
        tmp_encoding = np.concatenate(
            [
                word2vec.get_vector(word) if word in word2vec else []
                for word in stem_words(text)
            ]
        )
        if len(tmp_encoding) > embedding_size * max_text_len:
            ...
            encoding = np.concatenate(
                [
                    tmp_encoding,
                    np.zeros(
                        embedding_size * max_text_len - len(tmp_encoding)
                    ),
                ]
            )
    ...
    return (
        pd.DataFrame(
            x_train[TEXT_COLUMN_NAME].astype(str).apply(get_word2vec)
        ),
        pd.DataFrame(x_test[TEXT_COLUMN_NAME].astype(str).apply(get_word2vec)),
    )


def aggregative_word2vec(
    x_train: pd.DataFrame,
    x_test: pd.DataFrame,
    word2vec_path: str,
    embedding_size: int,
    ...
    of the words. (i.e mean, sum ...)
    @param word2vec_path: path to the word2vec encodings file
    @param embedding_size: size of each word embedding
    @param aggregative_function: the function to apply
    ...
    return (
        pd.DataFrame(
            x_train[TEXT_COLUMN_NAME].swifter.apply(get_aggrigated_word2vec)
        ),
        pd.DataFrame(
            x_test[TEXT_COLUMN_NAME].swifter.apply(get_aggrigated_word2vec)
        ),
    )
