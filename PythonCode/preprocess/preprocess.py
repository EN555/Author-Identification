...
def simple_style_handler(x_train, x_validation, x_test, *args, **kwargs):
    return simple_style_helper(x_train), simple_style_helper(x_test)


def bag_of_words(x_train,x_validation, x_test, *args, **kwargs):
    ...
    x_validation = pd.DataFrame(columns=vectorizer.get_feature_names(),
                          data=vectorizer.transform(x_validation[TEXT_COLUMN_LABEL]).toarray())
...
def preprocess_pipeline(data_path: str, data_path_test: str, number_of_authors: int, repesention_handler, normalize: bool, scaler=StandardScaler(),
                        test_size: float = 0.3,
    ...
    df_ = load_data(data_path_test, number_of_authors)  # test

    if data_filter is not None:
        df = data_filter(df)
    X, Y = pd.DataFrame(df[TEXT_COLUMN_LABEL], columns=[TEXT_COLUMN_LABEL]), df[AUTHOR_NAME_COLUMN_NAME]
    x_test, y_test = pd.DataFrame(df_[TEXT_COLUMN_LABEL], columns=[TEXT_COLUMN_LABEL]), df_[AUTHOR_NAME_COLUMN_NAME]

    x_train, x_validation, y_train, y_validation = train_test_split(X, Y, test_size=test_size)
    x_train, x_validation, x_test = repesention_handler(x_train, x_validation, x_test, **kwargs)
    ...
    y_validation = pd.factorize(y_validation)[0]
    ...
    if save_path is not None:
        ...
        x_validation.to_csv(os.path.join(save_path, "x_validation.csv"))
        y_train.to_csv(os.path.join(save_path, "y_train.csv"))
        y_validation.to_csv(os.path.join(save_path, "y_validation.csv"))
