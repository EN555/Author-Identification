...
def complex_style_features_extraction(x_train: pd.DataFrame, x_test: pd.DataFrame) -> (pd.DataFrame, pd.DataFrame):
    """
    @return: a representation of the data using complex stylistic features: lexicographic diversity and readability
    """
    def complex_style_features_extraction_helper(x: pd.DataFrame) -> pd.DataFrame:
        x = pd.concat(
            [honore_measure(x),
             hapax_disLegemena(x),
             ...
             gunning_fog_index(x)], axis=1)
        return x
    return complex_style_features_extraction_helper(x_train), complex_style_features_extraction_helper(x_test)


def characters_n_grams(x_train: pd.DataFrame, x_test: pd.DataFrame, n: int, **kwargs) -> (pd.DataFrame, pd.DataFrame):
    return bag_of_words(x_train, x_test, analyzer='char', ngram_range=(n, n), **kwargs)


def pos_n_grams(x_train: pd.DataFrame, x_test: pd.DataFrame, n: int, **kwargs) -> (pd.DataFrame, pd.DataFrame):
    pos_tagger = lambda text: ''.join([PENN2WN.get(pos, wn.NOUN) for _, pos in nltk.pos_tag(nltk.word_tokenize(text))])
    return bag_of_words(x_train, x_test, preprocessor=pos_tagger, ngram_range=(1, n), analyzer='char', **kwargs)
...
def Yules_characteristic(x: pd.DataFrame) -> pd.DataFrame:
    """
    Yules Characteristic (K) for lexicographic diversity: 10,000 * (M - N) / N**2, M = sum_i i**2 * vi
    N = number of words
    i =  word frequency
...
def flesch_reading_ease_flesch_kincaid_grade_level(x: pd.DataFrame) -> pd.DataFrame:
    """
    readability score using Flesch Reading Ease and the Flesch Kincaid Grade Level.
    """
    def get_flesch_reading_ease_flesch_kincaid_grade_level(text: str) -> (float, float):
        words = nltk.word_tokenize(text)
